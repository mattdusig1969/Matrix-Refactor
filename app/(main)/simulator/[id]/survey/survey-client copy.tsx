  ...
  const handleSaveSurvey = async () => {
    try {
      ...
      const { data: questionsData } = await supabase
        .from('questions')
        .select('question_text, question_type, answer_option, question_order')
        .eq('survey_id', params.id)
...
...`}
            ...
            <div className="space-y-4 mt-6">
              {/* Ensure parsedQuestions is an array before mapping */}
              {Array.isArray(parsedQuestions) && parsedQuestions.map((pq, idx) => (
                ...
                >
                  {editingIdx === idx ? (
                    ...
                    <>
                      <div className="font-bold mb-1">Q: {pq.question}</div>
