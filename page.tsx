                    ...
                    <div key={result.id || index} className="bg-white p-4 rounded shadow">
                        ...
                        <div className="mt-3 space-y-3 border-t pt-3">
                            {(result.answers || []).map((item, qIndex) => {
                                // FIX: Use strict integer comparison for lookup
    ...
    const getInitialData = async () => {
      ...
      try {
        ...
        const [surveyRes, countryRes, audienceRes] = await Promise.all([
          supabase.from('surveys').select('*, country(*)').eq('id', surveyId).single(),
        ...
        // Step 3: Fetch Questions and Existing Simulation Results
        const [questionsRes, resultsRes] = await Promise.all([
          supabase.from('questions').select('*').eq('survey_id', surveyId).order('question_number'),
          supabase.from('simulation_results').select('*').eq('survey_id', surveyId).order('respondent_number')
        ]);
        ...
        setQuestionsToAsk(questionsRes.data || []);
        setSimulationResults(resultsRes.data || []);

        if (resultsRes.data && resultsRes.data.length > 0) {
          toast.success(`Loaded ${resultsRes.data.length} existing results.`, { id: loadingToast });
        } else {
          toast.success("Survey data loaded. No existing results found.", { id: loadingToast });
        }
  ...
  const handleGenerateSimulation = async () => {
    ...
    for (let i = 1; i <= simulationCount; i++) {
      ...
      const prompt = `
        ...
        Here are the survey questions. Use the exact "question_number" provided for each question in your response.
        ---
        ${questionList}
        ---

        To ensure diversity, please create a persona that is distinct from the archetypes already generated: [${existingArchetypes}].
        
        Your response MUST be a single, valid JSON object. The root object should have a single key "simulated_responses" which is an array containing EXACTLY ONE object.
        This object represents one respondent and must have the following structure:
        {
          "respondent_number": ${respondentNumber},
          "archetype": a short, descriptive title for this persona (e.g., "Budget-Conscious Student", "Luxury Traveler"),
          "demographicProfile": an object summarizing the key demographic attributes of this persona based on the target audience provided,
          "answers": an array of objects, where each object has a "question_number" (matching the number from the input list) and an "answer" (as a string or array of strings for multiple_select).
        }
      `;
      ...
      try {
        ...
        const cleanedAnswers = questionsToAsk.map(question => {
            const aiAnswerObject = Array.isArray(newResult.answers) 
                ? newResult.answers.find(a => a.question_number === question.question_number) 
                : null;
            
            return {
                question_number: question.question_number, // Guaranteed to be a correct integer
